...
before_filter :authenticate_user!, :except => [:index, :show_review]
  def index
    @search_params = nil
    if params[:search]
      ...
      @search_params = @search_results.size
  ...
  def profile
    @user = current_user
    @post_requirements = @user.post_requirements.order( 'id DESC' ).paginate(:page => params[:page], :per_page => 1) 
    @book_post_requirements = @user.book_post_requirements.order( 'id DESC' ).paginate(:page => params[:page], :per_page => 1)
  ...
  def review
    # @review = Review.new
    # @book_post_requirement_id = params[:book_post_requirement_id]
    # @skill_post_requirement_id = params[:skill_post_requirement_id]
    # @post_requirement_id = params[:post_requirement_id]
  end
